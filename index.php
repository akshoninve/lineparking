...
// $selectedMonth всегда в формате "Месяц Год" (см. includes/parkings-data.php),
// поэтому год — это последнее слово строки. Передаём его в loadZayavkiLog(),
// чтобы читать только файл нужного года, а не всю многолетнюю историю
// заявок при каждом открытии страницы (см. комментарий в log-reader.php).
$selectedYear = substr($selectedMonth, strrpos($selectedMonth, ' ') + 1);

$entries = loadZayavkiLog($selectedYear);
